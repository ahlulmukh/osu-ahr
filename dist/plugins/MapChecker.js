...
exports.parseMapcheckerOwnerCommand =
  exports.MapValidator =
  exports.secToTimeNotation =
  exports.MapChecker =
    void 0;
...
class MapChecker extends LobbyPlugin_1.LobbyPlugin {
  ...
  registerEvents() {
    ...
    this.lobby.ReceivedChatCommand.on((a) =>
      this.onReceivedChatCommand(a.command, a.param, a.player)
    );
  ...
  processOwnerCommand(command, param) {
    try {
      ...
      if (p.num_violations_allowed !== undefined) {
        ...
        this.logger.info(
          `Number of allowed violations set to ${p.num_violations_allowed}`
        );
      }
      let changed = false;
      if (p.star_min !== undefined) {
        ...
        if (
          this.option.star_max <= this.option.star_min &&
      ...
      if (p.length_min !== undefined) {
        ...
        if (
          this.option.length_max <= this.option.length_min &&
      ...
      this.logger.warn(
        `@MapChecker#processOwnerCommand\n${e.message}\n${e.stack}`
      );
    }
  }
  ...
  async check(mapId, mapTitle) {
    ...
    try {
      const map = await BeatmapRepository_1.BeatmapRepository.getBeatmap(
        mapId,
        this.option.gamemode,
        this.option.allow_convert
      );
      if (mapId !== this.checkingMapId) {
        this.logger.info(
          `The target beatmap has already been changed. Checked beatmap: ${mapId}, Current: ${this.checkingMapId}`
        );
        return;
      }
      ...
      if (e instanceof BeatmapRepository_1.FetchBeatmapError) {
        switch (e.reason) {
          case BeatmapRepository_1.FetchBeatmapErrorReason.FormatError:
            this.logger.error(
              `Failed to parse the webpage. Checked beatmap: ${mapId}`
            );
            break;
          case BeatmapRepository_1.FetchBeatmapErrorReason.NotFound:
            this.logger.info(
              `Beatmap cannot be found. Checked beatmap: ${mapId}`
            );
            this.rejectMap(
              `[https://osu.ppy.sh/b/${mapId} ${mapTitle}] had already been removed from the website.`,
              false
            );
            break;
          case BeatmapRepository_1.FetchBeatmapErrorReason.PlayModeMismatched:
            ...
            this.rejectMap(
              `[https://osu.ppy.sh/b/${mapId} ${mapTitle}] is not ${this.option.gamemode.officialName} beatmap. Pick ${this.option.gamemode.officialName} beatmap.`,
              false
            );
            break;
          case BeatmapRepository_1.FetchBeatmapErrorReason.NotAvailable:
            this.logger.info(
              `Beatmap is not available. Checked beatmap: ${mapId}`
            );
            this.rejectMap(
              `[https://osu.ppy.sh/b/${mapId} ${mapTitle}] is not available for download.`,
              false
            );
            break;
        }
  ...
  rejectMap(reason, showRegulation) {
    ...
    this.logger.info(
      `Rejected the beatmap selected by ${this.lobby.host?.escaped_name} (${this.numViolations} / ${this.option.num_violations_allowed})`
    );
    if (showRegulation) {
      this.lobby.SendMessage(
        `!mp map ${this.lastMapId} ${
          this.option.gamemode.value
        } | Current regulation: ${this.validator.GetDescription()}`
      );
      ...
      this.lobby.SendMessage(
        `!mp map ${this.lastMapId} ${this.option.gamemode.value} | ${reason}`
      );
    }
    this.checkingMapId = 0;
    if (
      this.option.num_violations_allowed !== 0 &&
      this.option.num_violations_allowed <= this.numViolations
    ) {
      this.skipHost();
    }
  }
  acceptMap(map) {
    ...
    if (map.beatmapset) {
      ...
      const rating = this.getRating(map, map.beatmapset);
      const downloadMap = this.getDownload(map, map.beatmapset);
      this.lobby.SendMessage(
        `!mp map ${this.lobby.mapId} ${this.option.gamemode.value} | ${desc}`
      );
      this.lobby.SendMessage(`${rating}`);
      this.lobby.SendMessage(`${downloadMap}`);
    } else {
      this.lobby.SendMessage(
        `!mp map ${this.lobby.mapId} ${this.option.gamemode.value}`
      );
    }
  }
  ...
  getRating(map, set) {
    let rating = this.option.map_rating;
    rating = rating.replace(/\$\{star\}/g, map.difficulty_rating.toFixed(2));
  ...
  getDownload(map, set) {
    let download = this.option.map_download;
    download = download.replace(/\$\{beatmapset_id\}/g, set.id.toString());
    return download;
  }
  GetPluginStatus() {
    return `-- Map Checker --
...
class MapValidator {
  ...
  RateBeatmap(map) {
    ...
    if (mapmode !== this.option.gamemode && this.option.gamemode !== null) {
      violationMsgs.push(
        `the gamemode is not ${this.option.gamemode.officialName}.`
      );
      rate += 1;
    }
    if (
    ...
    if (
      this.option.star_max > 0 &&
      this.option.star_max < map.difficulty_rating
    ) {
      rate += parseFloat(
    ...
    if (
      this.option.length_min > 0 &&
      map.total_length < this.option.length_min
    ) {
      rate += (this.option.length_min - map.total_length) / 60.0;
      violationMsgs.push(
        "the beatmap length is shorter than the allowed length."
      );
    }
    if (
      this.option.length_max > 0 &&
      this.option.length_max < map.total_length
    ) {
      rate += (map.total_length - this.option.length_max) / 60.0;
      violationMsgs.push(
        "the beatmap length is longer than the allowed length."
      );
    }
    if (rate > 0) {
      let message;
      const mapDesc = `[${map.url} ${map.beatmapset?.title}] (Star rating: ${
        map.difficulty_rating
      }, Length: ${secToTimeNotation(map.total_length)})`;
  ...
  GetDescription() {
    ...
    if (this.option.star_min > 0 && this.option.star_max > 0) {
      d_star = `${this.option.star_min.toFixed(
        2
      )} <= star rating <= ${this.option.star_max.toFixed(2)}`;
    ...
    if (this.option.length_min > 0 && this.option.length_max > 0) {
      d_length = `${secToTimeNotation(
        this.option.length_min
      )} <= length <= ${secToTimeNotation(this.option.length_max)}`;
...
function validateMapCheckerOption(option) {
  if (option.enabled !== undefined) {
    option.enabled = OptionValidator_1.validateOption.bool(
      "MapChecker.enabled",
      option.enabled
    );
  }
  if (option.star_min !== undefined) {
    option.star_min = OptionValidator_1.validateOption.number(
      "MapChecker.star_min",
      option.star_min,
      0
    );
  ...
  if (option.star_max !== undefined) {
    option.star_max = OptionValidator_1.validateOption.number(
      "MapChecker.star_max",
      option.star_max,
      0
    );
  ...
  if (option.length_min !== undefined) {
    option.length_min = OptionValidator_1.validateOption.number(
      "MapChecker.length_min",
      option.length_min,
      0
    );
  }
  if (option.length_max !== undefined) {
    option.length_max = OptionValidator_1.validateOption.number(
      "MapChecker.length_max",
      option.length_max,
      0
    );
  }
  if (
    option.star_max !== undefined &&
    option.star_min !== undefined &&
    option.star_max <= option.star_min &&
    option.star_max > 0
  ) {
    option.star_min = 0;
  }
  if (
    ...
    option.length_max <= option.length_min &&
    option.length_max > 0
  ) {
    option.length_min = 0;
  }
  if (option.gamemode !== undefined) {
    if (typeof option.gamemode === "string") {
      try {
        ...
        throw new Error(
          "MapChecker#validateMapCheckerOption: Option must be [osu | fruits | taiko | mania]"
        );
      }
    }
  ...
  if (option.num_violations_allowed !== undefined) {
    option.num_violations_allowed = OptionValidator_1.validateOption.number(
      "MapChecker.num_violations_allowed",
      option.num_violations_allowed,
      0
    );
  }
  ...
  if (option.allow_convert !== undefined) {
    option.allow_convert = OptionValidator_1.validateOption.bool(
      "MapChecker.allow_convert",
      option.allow_convert
    );
  }
  return true;
}
...
function parseRegulationCommand(params) {
  switch (unifyParamName(params[0])) {
    ...
    case "num_violations_allowed":
      if (params.length < 2)
        throw new Error(
          "Missing parameter. *regulation num_violations_allowed [number]"
        );
    ...
    case "gamemode":
      if (params.length < 2)
        throw new Error(
          "Missing parameter. *regulation gamemode [osu | fruits | taiko | mania]"
        );
  ...
  throw new Error(
    "Missing parameter. *regulation [enable | disable | star_min | star_max | length_min | length_max | gamemode | num_violations_allowed] <...params>"
  );
}

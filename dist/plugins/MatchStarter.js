...
class MatchStarter extends LobbyPlugin_1.LobbyPlugin {
  constructor(lobby, option = {}) {
    ...
    this.voting = new VoteCounter_1.VoteCounter(
      this.option.vote_rate,
  ...
  registerEvents() {
    ...
    this.lobby.ReceivedChatCommand.on((a) =>
      this.onChatCommand(a.player, a.command, a.param)
    );
    this.lobby.PluginMessage.on((a) =>
      this.onPluginMessage(a.type, a.args, a.src)
    );
    ...
    switch (command) {
      case "!start":
        if (param === "") {
          ...
          (player.isHost || player.isAuthorized) &&
          param.match(/^\d+$/)
        ) {
          this.startTimer(parseInt(param));
          this.lobby.SendMessage("type !abort to cancel timer");
        }
        break;
      case "!stop":
      case "!abort":
  ...
  vote(player) {
    ...
    if (this.voting.Vote(player)) {
      this.logger.trace(
        `Accepted a match start request from player ${player.name}`
      );
  ...
  checkVoteCount(showMessage = false) {
    if (this.voting.count !== 0 && showMessage) {
      this.lobby.DeferMessage(
        `Bot: Match start progress: ${this.voting.toString()}`,
        "match start vote",
        this.option.vote_msg_defer_ms,
        false
      );
    }
    if (this.voting.passed) {
      this.lobby.DeferMessage(
        `Bot: Passed a match start vote: ${this.voting.toString()}`,
        "match start vote",
        100,
        true
      );
  ...
  startTimer(count, withHint = false) {
    if (count === 0) {
      ...
      this.lobby.SendMessage(
        `Queued the match to start in ${this.secsToCountdownText(count)}${
          withHint ? ". (Host can stop the timer with !stop command.)" : ""
        }`
      );

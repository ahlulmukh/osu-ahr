...
class Lobby {
  constructor(ircClient, option = {}) {
    ...
    this.transferHostTimeout = new DeferredAction_1.DeferredAction(() =>
      this.onTimeoutedTransferHost()
    );
    this.registerEvents();
  }
  registerEvents() {
    this.events = {
      ...
      registered: async () => {
        if (this.status === LobbyStatus.Entered && this.channel) {
          this.logger.warn(
            "Detected a network reconnection! Loading multiplayer settings..."
          );
  ...
  get playersFinished() {
    ...
    for (const p of this.players) {
      if (p.mpstatus === Player_1.MpStatuses.Finished) i++;
    }
    return i;
  }
  /**
  ...
  get playersInGame() {
    ...
    for (const p of this.players) {
      if (
        p.mpstatus === Player_1.MpStatuses.Finished ||
        p.mpstatus === Player_1.MpStatuses.Playing
      )
        i++;
    }
    return i;
  ...
  DeferMessage(message, tag, delayMs, resetTimer = false) {
    ...
    if (!(tag in this.deferredMessages)) {
      this.deferredMessages[tag] = new DeferredAction_1.DeferredAction(
        (msg) => {
  ...
  async RequestStatAsync(player, byPm, timeout = this.option.stat_timeout_ms) {
    return new Promise((resolve, reject) => {
      ...
      this.ircClient.say(
        byPm || !this.channel ? "BanchoBot" : this.channel,
        `!stat ${player.escaped_name}`
      );
    });
  }
  ...
  handleBanchoResponse(message) {
    ...
    switch (c.type) {
      ...
      case CommandParser_1.BanchoResponseType.TeamChanged:
        ...
        this.logger.trace(
          `Team has been changed: ${c.params[0]}, ${
            Player_1.Teams[c.params[1]]
          }`
        );
        break;
      ...
      case CommandParser_1.BanchoResponseType.MpBeatmapChanged:
        if (this.mapId !== c.params[0]) {
          ...
          const changer = this.host
            ? `by ${
                c.type === CommandParser_1.BanchoResponseType.BeatmapChanged
                  ? this.host.name
                  : "Bot"
              }`
            : "";
          this.logger.info(
            `Beatmap has been changed ${changer}: https://osu.ppy.sh/b/${this.mapId} ${this.mapTitle}`
          );
        }
        break;
  ...
  RaiseReceivedChatCommand(player, message) {
    ...
    if (command === "!gpt" || command === "!ai") {
      const userText = param.trim();
      if (userText) {
        this.SendMessageToAI(userText, this.SendAIAIResponse.bind(this));
      } else {
  ...
  RaisePlayerMoved(username, slot) {
    ...
    player.slot = slot;
    this.logger.trace(
      `A slot has been moved. Player: ${username}, Slot: ${slot}`
    );
  ...
  RaisePlayerFinished(username, score, isPassed) {
    ...
    this.PlayerFinished.emit({
      ...
      score,
      isPassed,
      playersFinished: sc.finished,
      playersInGame: sc.inGame,
    });
    if (!this.players.has(player)) {
      this.logger.warn(
        `A player that did not participate finished a match: ${username}`
      );
      this.LoadMpSettingsAsync();
    }
  }
  ...
  RaiseJoinedLobby(channel) {
    ...
    this.JoinedLobby.emit({
      channel: this.channel,
      creator: this.GetOrMakePlayer(this.ircClient.nick),
    });
  ...
  RaiseParsedSettings() {
    if (!this.settingParser.isParsing && this.settingParser.result) {
      ...
      if (
        r.hostChanged ||
        r.playersIn.length !== 0 ||
        r.playersOut.length !== 0
      ) {
  ...
  RaiseParsedStat(isPm) {
    if (!this.statParser.isParsing && this.statParser.result) {
      ...
      if (p) {
        ...
        this.logger.info(
          `Parsed a player's stat: ${p.name} -> ${
            StatParser_1.StatStatuses[p.laststat.status]
          }`
        );
        this.ParsedStat.emit({
          result: this.statParser.result,
          player: p,
          isPm,
        });
      }
  ...
  EnterLobbyAsync(channel) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (ch === "") {
        this.logger.error(
          `@Lobby#enterLobbyAsync: Invalid channel specified: ${channel}`
        );
  ...
  LoadMpSettingsAsync() {
    ...
    if (this.SendMessageWithCoolTime("!mp settings", "mpsettings", 15000)) {
      ...
      this.logger.trace(
        "Multiplayer settings loading process has been skipped due to cooltime."
      );
  ...
  addPlayer(player, slot, team, asHost = false) {
    ...
    if (!this.players.has(player)) {
      ...
      if (this.players.size > 16) {
        this.logger.warn(
          `A player has joined the lobby with more than 16 players: ${player.name}`
        );
        this.UnexpectedAction.emit(
          new Error("A player has joined the lobby with more than 16 players")
        );
        return false;
      }
      return true;
      ...
      this.logger.warn(
        `A player inside the lobby has joined for the second time: ${player.name}`
      );
      this.UnexpectedAction.emit(
        new Error("A player inside the lobby has joined for the second time")
      );
      return false;
    }
  }
  removePlayer(player) {
    ...
    if (this.players.has(player)) {
      ...
      this.UnexpectedAction.emit(
        new Error("A player outside the lobby has left")
      );
      return false;
    }
  }
  setAsHost(player) {
    if (!this.players.has(player)) {
      this.transferHostTimeout.cancel();
      this.logger.warn(
        `A player outside the lobby became a host: ${player.name}`
      );
      return false;
    }
    if (this.hostPending === player) {
      ...
      this.logger.warn(
        `Another player became host during host assignment. Pending: ${this.hostPending.name}, Host: ${player.name}`
      );
...
=== Lobby Status ===
  ...
  Player(s): ${this.players.size} (In-game: ${pc.inGame}, Playing: ${
      pc.playing
    })
  Referee(s): ${Array.from(this.playersMap.values())
    .filter((v) => v.isReferee)
    .map((v) => v.name)
    .join(",")}
  ...
  showInfoMessage() {
    this.SendMessageWithCoolTime(
      this.getInfoMessage(),
      "infomessage",
      this.option.info_message_cooltime_ms
    );
  }
  SendMessageToAI(message, callback) {
    const apiUrl = "https://skizo.tech/api/openai?apikey=ahmuq";
    const messages = [
      { role: "system", content: "Adjust your speaking style to look cool" },
  ...
  SendAIAIResponse(response) {
    this.SendMessage(response);
  }
  showVersionMessage() {
    const version = this.tryGetVersion();
    this.SendMessageWithCoolTime(
      `osu! Auto Host Rotation Bot v. ${version}`,
      "versionmessage",
      this.option.info_message_cooltime_ms
    );
  }
  sendInfoMessagePM(player) {
    this.SendPrivateMessageWithCoolTime(
      this.getInfoMessage(),
      player.escaped_name,
      "infomessage",
      this.option.info_message_cooltime_ms
  ...
  startInfoMessageAnnouncement() {
    ...
    if (this.option.info_message_announcement_interval_ms > 3 * 60 * 1000) {
      this.logger.trace(
        `Started info message announcement timer. Interval: ${this.option.info_message_announcement_interval_ms}`
      );
